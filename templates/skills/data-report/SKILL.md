...
- **硬性视觉签名 — 贴近 example.html**:
  - 背景用暖白/纸色 `#fafaf7`, 文字 `#15140f`, 次级文字 `#5a564e`, 细线 `#e7e5e0`。
  - 主强调色用陶土橙 `#c96442`; 正向/负向只用少量绿色 `#1f7a3a` 与红色 `#9c2a25`。
  - 卡片是白色或暖白, 1px 暖灰边框, 轻微 shadow, 圆角 16px 左右; 不要玻璃拟态、霓虹、蓝紫 SaaS 渐变。
  - 标题区要有小号 uppercase/letter-spacing 标签, 大标题紧凑有重量; 数字可用 mono。
  - 图表配色从陶土橙、芥末黄、深蓝、绿中选择, 不要使用默认 Chart.js 彩虹色。
- **必须解析用户提供的实际数据**, 不要捏造。
