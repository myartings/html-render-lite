...
- 图形：Hero 场景 + case setup + 五张不同形态的 SVG 图 + walkthrough + boundary。图形语言要包含场景卡、剖面图、反馈环、位置拓扑、因果链，不能五张图长得一样。
...
6. `动态机制图` 和 `环境动力图` 是否被清楚区分？
7. 页面是否比 Markdown 更适合第一次理解和复习？
8. 手机端是否无横向溢出？
9. 是否保留了 skill 的方法论价值，而不是只做漂亮图？
